...
- **XPath-based targeting** — target elements are identified via "Copy XPath" from the browser DevTools
...
- **Customizable banner texts** — notification, button labels and dismiss text are part of the JSON configuration
...
| `xpath` | XPath expression targeting the element (via DevTools → Copy XPath) |
...
3. In the DevTools panel: right-click the highlighted HTML tag → **Copy** → **Copy XPath**
4. Paste the XPath into the configuration under `"xpath"`
...
└── Koha/Plugin/Com/MarkusMajer/
    ...
    └── KohaTour/
        ...
        ├── js/tour.js           # Client-side tour runner
        └── templates/
...
- **`intranet_js`** — injects on every staff page: driver.js (CDN), the tour configuration as an inline JSON object (`window.KOHA_TOUR_CONFIG`), and `tour.js`
...
Runs on every staff page. Compares `window.location.pathname` against configured paths. On match, a banner is displayed with texts from the `banner` object in the configuration. Uses [driver.js](https://driverjs.com/) for the tour overlay with XPath resolution.
